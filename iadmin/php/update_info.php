...
$more_info_html = $_POST['more_info_html'];
...
// #workspace_settings_page
        ...
        // Update mentality trends
            $ment_trends = explode('-,-', $ment_trends);
            $ment_trends_db = '';
            $sql = mysql_query("SELECT * FROM mentality_trends WHERE workspace_id='$workspace_id'");
            while($row = mysql_fetch_array($sql)){
                $ment_trends_db[] = $row['name'];
            }
            ...
            // Add ment trend
                foreach($to_add as $ment_trend){
                    mysql_query("INSERT INTO mentality_trends (name, workspace_id) VALUES ('$ment_trend', '$workspace_id')") or die(mysql_error());
                }

            // Delete ment trend
                foreach($to_del as $ment_trend){
                    mysql_query("DELETE FROM mentality_trends WHERE name='$ment_trend' AND workspace_id='$workspace_id'") or die(mysql_error());
                }
            ...
            // If added a ment trend, order it
                if(json_encode($to_add)){
                    for($i=0; $i<count($ment_trends); $i++){
                        ...
                        mysql_query("UPDATE mentality_trends SET order_id='$order' WHERE name='$ment_trend' AND workspace_id='$workspace_id'") or die(mysql_error());

                    }
                }

        // Update more info
            mysql_query("UPDATE research_projects SET more_info_html='$more_info_html' WHERE id='$workspace_id'");
